...
//    NSString *MIMEType = RKMIMETypeFromPathExtension(path);
    id parsedData = nil;//[RKMIMETypeSerialization objectFromData:payload MIMEType:MIMEType error:&localError];
    if (!parsedData) {
...
- (NSUInteger)importObjectsFromDirectoryAtPath:(NSString *)path withMapping:(RKMapping *)mapping keyPath:(NSString *)keyPath error:(NSError **)error
{
    ...
    for (NSString *entry in entries) {
        ...
        NSUInteger objectCount = [self importObjectsFromFileAtPath:entryPath withMapping:mapping keyPath:keyPath error:&localError];
