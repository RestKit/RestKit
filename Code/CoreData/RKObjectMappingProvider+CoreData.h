 ...
 **NOTE** that care must be taken when configuring patterns within the provider. The patterns
 ...
 @see RKObjectLoader
 */
- (void)setObjectMapping:(id<RKObjectMappingDefinition>)objectMapping forResourcePathPattern:(NSString *)resourcePathPattern withFetchRequestBlock:(RKObjectMappingProviderFetchRequestBlock)fetchRequestBlock;
