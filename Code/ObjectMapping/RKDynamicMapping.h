...
/**
 ...
 1. Through the use of a mapping selection block configured by `setObjectMappingForRootParentRepresentationBlock:`. When configured, the block is called with a reference to the current object's root parent representation and is expected to return an `RKObjectMapping` object. Returning `nil` declines the mapping of the representation.
...
/**
 Sets a block to be invoked to determine the appropriate concrete object mapping with which to map an object representation according to root parent representation.
 
 @param block The block object to invoke to select the object mapping with which to map linked object representation. The block returns an object mapping and accepts a single parameter: the object's root parent representation.
 */
- (void)setObjectMappingForRootParentRepresentationBlock:(RKObjectMapping *(^)(id rootParentRepresentation))block;
...
/**
 ...
 This method searches the stack of registered matchers, then executes the block, if any, set by `setObjectMappingForRepresentationBlock:`. If `nil` is returned, then mapping for the representation is declined and it will not be mapped.
