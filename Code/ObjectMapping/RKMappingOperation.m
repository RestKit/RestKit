...
@interface RKMappingMetadataWrapper : NSObject
- (id)initWithMetadataList:(NSArray *)list;
@property (nonatomic, strong) NSArray *metadataList;
@end
@implementation RKMappingMetadataWrapper
- (id)initWithMetadataList:(NSArray *)list {
    if (self = [super init]) {
...
- (id)valueForKey:(NSString *)key
{
    ...
    if (firstChar == 's' && [key isEqualToString:RKSelfKey]) {
        ...
        return [[RKMappingMetadataWrapper alloc] initWithMetadataList:self.metadataList];
...
{
    return [self initWithSourceObject:sourceObject destinationObject:destinationObject mapping:objectOrDynamicMapping metadata:nil];
}
...
- (id)destinationObjectForMappingRepresentation:(id)representation parentRepresentation:(id)parentRepresentation withMapping:(RKMapping *)mapping inRelationship:(RKRelationshipMapping *)relationshipMapping
{
    ...
    if (destinationObject == nil)
    {
        ...
        NSArray *metadata = RKInsertInDictionaryList(self.metadataList, @{ @"mapping": @{ @"parentObject": (self.destinationObject ?: [NSNull null]) } }, nil, nil);
...
- (void)cacheMappingsIfNeeded
{
    if (!_nestedAttributeMappings)
    {
        if (self.nestedAttributeSubstitutionKey == nil) {
            ...
            _nestedAttributeMappings = [self applyNestingToMappings:self.objectMapping.attributeMappings];
            _relationshipMappings = [self applyNestingToMappings:self.objectMapping.relationshipMappings];
    ...
    for (RKAttributeMapping *attributeMapping in attributeMappings) {
        ...
        if ([self applyAttributeMapping:attributeMapping withValue:value]) {
            ...
            if ([self.delegate respondsToSelector:@selector(mappingOperation:didNotFindValueForKeyPath:mapping:)]) {
                [self.delegate mappingOperation:self didNotFindValueForKeyPath:sourceKeyPath mapping:attributeMapping];
            }
            ...
            if (self.objectMapping.assignsDefaultValueForMissingAttributes) {
                [self.destinationObject setValue:[self.objectMapping defaultValueForAttribute:destinationKeyPath]
...
- (BOOL)mapNestedObject:(id)anObject toObject:(id)anotherObject parent:(id)parentSourceObject withRelationshipMapping:(RKRelationshipMapping *)relationshipMapping metadata:(NSDictionary *)metadata
{
    ...
    NSArray *subOperationMetadata = RKInsertInDictionaryList(self.metadataList, metadata, nil, nil);
    RKMappingOperation *subOperation = [[RKMappingOperation alloc] initWithSourceObject:anObject destinationObject:anotherObject mapping:relationshipMapping.mapping metadata:subOperationMetadata];
...
- (BOOL)applyRelationshipMappings
{
    ...
    for (RKRelationshipMapping *relationshipMapping in [self relationshipMappings]) {
        ...
        if (sourceKeyPath) {
            value = [self.sourceObject valueForKeyPath:sourceKeyPath];
        } else {
        ...
        if (value == nil) {
            ...
            if ([self shouldSetValue:&value forKeyPath:destinationKeyPath usingMapping:relationshipMapping]) {
                ...
                [self.destinationObject setValue:value forKeyPath:destinationKeyPath];
            }
        ...
        if ([self.delegate respondsToSelector:@selector(mappingOperation:didSetValue:forKeyPath:usingMapping:)]) {
            id setValue = [self.destinationObject valueForKeyPath:destinationKeyPath];
            [self.delegate mappingOperation:self didSetValue:setValue forKeyPath:destinationKeyPath usingMapping:relationshipMapping];
        }
