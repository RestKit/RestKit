...
- (void)addRoute:(RKRoute *)route
{
    ...
    if ([route isClassRoute]) {
        ...
        if (! (existingRoute == nil || (existingRoute.method == RKHTTPMethodAny && route.method != RKHTTPMethodAny) || (route.method == RKHTTPMethodAny && existingRoute.method != RKHTTPMethodAny))) [NSException raise:NSInternalInconsistencyException format:@"Cannot add a route with the same class and method as an existing route."];
...
- (RKRoute *)routeForClass:(Class)objectClass method:(RKHTTPMethodOptions)method
{
    // Check for an exact match
    for (RKRoute *route in [self classRoutes]) {
        if ([route.objectClass isEqual:objectClass] && (route.method != RKHTTPMethodAny && route.method & method)) {
            return route;
        }
    }
    ...
    for (RKRoute *route in [self classRoutes]) {
        if ([route.objectClass isEqual:objectClass] && route.method == RKHTTPMethodAny) {
            return route;
        }
    }
...
- (RKRoute *)routeForRelationship:(NSString *)relationshipName ofClass:(Class)objectClass method:(RKHTTPMethodOptions)method
{
    for (RKRoute *route in [self relationshipRoutes]) {
        if ([route.name isEqualToString:relationshipName] && [route.objectClass isEqual:objectClass] && (route.method == method || route.method == RKHTTPMethodAny)) {
            return route;
        }
    }
...
- (RKRoute *)routeForObject:(id)object method:(RKHTTPMethodOptions)method
{
    ...
    while (searchClass) {
        ...
        for (RKRoute *route in routes) {
            ...
            if (route.method == RKHTTPMethodAny) wildcardRoute = route;
