...
/**
 ...
 @see `RKRouter`
 */
@interface RKRouteSet : NSObject <NSCopying>
 ...
 */
- (RKRoute *)routeForRelationship:(NSString *)relationship ofClass:(Class)objectClass method:(RKHTTPMethodOptions)method;
...
/**
 ...
 The object routes are first searched for an exact match with the given object's class and request method. If no exact match is found for the given request method, but a route is found for the `RKHTTPMethodAny` method, it is returned. If neither are found, the search process begins again and traverses up the inheritance hierarchy.
 */
