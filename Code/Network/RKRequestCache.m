...
- (id)initWithCachePath:(NSString*)cachePath storagePolicy:(RKRequestCacheStoragePolicy)storagePolicy {
    self = [super init];
	if (self) {
        _cache = [[RKCache alloc] initWithCachePath:cachePath
...
- (NSString*)cachePathForRequest:(RKRequest*)request {
	NSString* cachePathForRequest = nil;
    NSString* requestCacheKey = [request cacheKey];
    if (requestCacheKey) {
        if (_storagePolicy == RKRequestCacheStoragePolicyForDurationOfSession) {
            cachePathForRequest = [sessionCacheFolder stringByAppendingPathComponent:requestCacheKey];

        } else if (_storagePolicy == RKRequestCacheStoragePolicyPermanently) {
            cachePathForRequest = [permanentCacheFolder stringByAppendingPathComponent:requestCacheKey];
        }
        RKLogTrace(@"Found cacheKey '%@' for %@", cachePathForRequest, request);
    } else {
        RKLogTrace(@"Failed to find cacheKey for %@ due to nil cacheKey", request);
    }
	return cachePathForRequest;
}

- (BOOL)hasResponseForRequest:(RKRequest*)request {
	BOOL hasEntryForRequest = NO;
	NSString* cacheKey = [self cachePathForRequest:request];
    if (cacheKey) {
